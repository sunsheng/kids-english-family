...
if (!email || !password) {
  console.error("用法: node scripts/create-user.mjs <email> <password> [nickname]");
  console.error("示例: node scripts/create-user.mjs parent@example.com mypassword 张三");
  process.exit(1);
}
...
async function createUser() {
  const pool = new Pool({
    connectionString:
      ...
      `postgres://kids_english:kids_english_dev@localhost:5432/kids_english_family`,
  });
  ...
  try {
    ...
    // 3. 创建学生统计记录
    await pool.query(`INSERT INTO student_stats (student_id) VALUES ($1)`, [student.id]);
