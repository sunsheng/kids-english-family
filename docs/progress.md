...
- 2026-07-09（测试只考已认识的词）：
  1. **测试选词规则收紧**：`/api/testing/next` 只出「学过且最近一次被标记认识/答对」的词(`learning_records.repetitions > 0`)。没学过的词、点了「不认识」或复习答错的词(repetitions 归零)自动跳过——本来就不会的词测了没有意义。测试游标机制不变,仍与学习进度隔离。
  2. **空态提示细分**：接口无词可出时返回 `reason`(`no_book` / `no_learned_words` / `completed`),测试页对应提示「还没有选择词书」「还没有可测试的单词：先去学习室学一学」「当前词书的测试已全部完成」,不再笼统显示已完成。e2e 同步覆盖:只点过「不认识」时测试无词可出,点「认识」后该词才进入测试。
