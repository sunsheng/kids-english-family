...
- 2026-07-09（测试只考已认识的词）：
  1. **测试选词规则收紧**：`/api/testing/next` 只出「学过且最近一次被标记认识/答对」的词(`learning_records.repetitions > 0`)。没学过的词、点了「不认识」或复习答错的词(repetitions 归零)自动跳过——本来就不会的词测了没有意义。
  2. **测试进度从「单向游标」改为「按词记录」**：新建 `test_records` 表(学员 x 词书 x 单词),替代 `test_progress` 游标(旧表保留作历史数据,代码不再读写,旧游标数据自动迁移)。测对过的词不再重复出现;当初点「不认识」被跳过的词,之后复习学会了会按词书顺序自动补测;测错的词重新学会后同样回到测试队列。应用启动迁移(`lib/db-bootstrap.ts`)支持在备份恢复之后按版本增量执行 `db/init` 新脚本。
  3. **空态提示细分**：接口无词可出时返回 `reason`(`no_book` / `no_learned_words` / `completed`),测试页对应提示「还没有选择词书」「还没有可测试的单词：先去学习室学一学」「已学会的单词都测完了」。e2e 同步覆盖:只点过「不认识」时测试无词可出;点「认识」后该词才进入测试;「不认识」的词复习答对后回到测试页可补测。
- 2026-07-08（多单词拼写测试修复 + 去 Docker 化）：
  1. **修复多单词词条空格无法输入的 bug**：`ice cream` 这类词条原先把空格也当成待填字母,但输入层只放行 a-z,空格永远输不进去,测试卡死。现在空格、连字符、撇号等非字母字符统一按「固定分隔符」处理——始终显示、永不挖空、判分时剔除;填完前一个单词的挖空后自动落到下一个单词,无需输入空格。挖空长度/位置改按去掉分隔符后的字母序列计算(`lib/spelling-drill.ts`),分隔符渲染为窄分隔格(`.letter-cell.separator`),新增 e2e 用例覆盖多单词词条全流程。
  2. **开发环境去 Docker 化**：删除 `docker-compose.yml`,本地开发改为直接使用本机 PostgreSQL(线上继续用 Vercel + Neon),README 与测试文档同步更新。
